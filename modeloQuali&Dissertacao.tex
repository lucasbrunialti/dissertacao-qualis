...
% diferentes técnicas de cada algoritmo ??? como assim ??? Não entendi o que voce quis dizer.
%PESAGEM? Não é melhor deixar em ingles mesmo?
...
%Lucas, melhore um pouquinho a introdução com base nisso que você tem aqui. Se ficar um pouco repetitivo, não tem problema. Melhor reclamarem que está repetindo do que reclamarem que está ruim. Embora eu ainda ache que você deva citar o Fabrício na introdução e em outros lugares do texto.

%Outro problema. Na introdução eu comentei sobre agrupamento, em alguns lugares. Vc está usando clusterização. Acho que precisa padronizar. Escolha uma forma e arrume no texto todo.

% veja o que vc decidiu na introdução sobre a notação e a depender do que decidiu lá veja se tem que arrumar aqui.

Para a descrição do problema formal de biclusterização usa-se a seguinte definição \cite{Madeira2004}: seja uma matriz $A$, $n \times m$, um conjunto de linhas $X = \{ x_1, \dots, x_n \}$ e um conjunto de colunas $Y = \{ y_1, \dots, y_m \}$, em que $a_{ij}$ geralmente é um número real e representa a relação entre a linha $x_i$ e a coluna $y_j$; o problema de biclusterização é encontrar biclusters, que são submatrizes de $A$, denotados por $A_{IJ}$, em que $I \subseteq X$ e $J \subseteq Y$.
...
%Lucas, eu gostaria de revisar todas essa formalização com um pouco mais de cuidado. Mas não vai ser possível agora, pois eu preciso acessar os artigos fontes e olhar com cuidado. Por favor, nos próximos textos, me dê a chance de trabalhar com mais calma, tá? PLEASE!!!!!!!
