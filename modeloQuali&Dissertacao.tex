...
\begin{resumo}
\begin{flushleft}
\noindent BRUNIALTI, Lucas Fernandes. \textbf{Biclusterização aplicada em Sistemas de Recomendação baseados em Conteúdo Textual}. 2015. 59 f. Dissertação (Mestrado em Ciências) -- Escola de Artes, Ciências e Humanidades, Universidade de São Paulo, São Paulo, 2015.
\newline
\end{flushleft}
...
Esta necessidade configura-se como um problema de serendipidade, uma das propriedades de Sistemas de Recomendação.
O objetivo deste trabalho é analisar a aderência dos resultados provenientes de algoritmos de Biclusterização, aplicados aos itens a serem recomendados, à meta de otimização da serendipidade.
...
Como forma de avaliação dos resultados obtidos, é proposta uma análise comparativa entre os resultados da recomendação obtidos via Biclusterização e os resultados de recomendação obtidos via filtro colaborativo, onde o problema da serendipidade é reconhecidamente bem resolvido.
\end{resumo}
...
\begin{abstract}
\begin{flushleft}
\noindent BRUNIALTI, Lucas Fernandes. \textbf{Biclustering applied to Content Based Recommender Systems in Textual Domain}. 2015. 59 p. Dissertation (Master of Science) -- School of Arts, Sciences and Humanities, University of São Paulo, São Paulo, 2015.
\newline
\end{flushleft}
...
Dada a quantidade e dinamicidade de informação presente na \textit{web}, é comum as pessoas, usuárias da \textit{web}, se verem diante de um problema de difícil resolução quando querem encontrar alguma informação.
...
Considerando especificamente a ocorrência desse problema no domínio de notícias, os portais de notícias da \textit{web} começam a perceber a necessidade do oferecimento de conteúdo personalizado, e portanto, da construção de um SR.
...
O contexto apresentado leva à discussão de uma característica de recomendação denominada serendipidade.
...
\section{Apresentação do Problema de Recomendação baseada em Conteúdo}
...
A idéia é que a lista de recomendações $L_{u_n}$ permita a ocorrência de serendipidade, incluindo um fator de ponderação na função de similaridade $s$, que considere similaridades parciais.
...
Com maior formalidade, o problema de Biclusterização consiste em uma matriz $A$, de dimensão $N \times M$, um conjunto de linhas (objetos) $X = \{ x_1, \dots, x_n, \dots, x_N \}$ e um conjunto de colunas (atributos) $Y = \{ y_1, \dots, y_m, \dots, y_M \}$, em que $a_{nm}$, geralmente um número real, e representa a relação entre a linha $x_n$ e a coluna $y_m$ \cite{Franca2010,Mirkin1996,Madeira2004}.
...
A análise exploratória da literatura especilizada foi escolhida como estratégia para a aquisição de conhecimento sobre a área de SR, Biclusterização e Mineração de Texto.
Já para o levantamento do estado da arte em SRs baseados em conteúdo textual e sobre a aplicação de técnicas de Aprendizado de Máquina, mais especificamente clusterização, em SRs baseados em conteúdo textual, optou-se por aplicar a técnica de construção de revisões sistemáticas.
...
A RS que relaciona Aprendizado de Máquina e SRs se faz útil uma vez que a aplicação de Biclusterização como estratégia de resolução do problema é o núcleo deste projeto.
Apesar de estar sendo realizada uma RS, neste documento optou-se por apresentar uma revisão bibliográfica no formato simples, isto é, sem os processos detalhados em \citeonline{Kitchenham2004}, visto que a construção da RS ainda esta em andamento.
...
Os resultados da aplicação dos algoritmos de análise de texto via Biclusterização deverão ser validados utilizando técnicas de avaliação interna, para a verificação da consistência dos biclusters encontrados \cite{Santamaria2007}, e externas \cite{Hochreiter2010}, avaliando o quanto os biclusters encontrados estão em consenso com as classes de notícias \cite{Hochreiter2010}.
...
Este documento é composto por quatro capítulos incluindo esta introdução. No Capítulo 2 conceitos fundamentais necessários para a compreensão deste projeto são apresentados, incluindo conceitos sobre SRs, Biclusterização e mineração de texto; o Capítulo 3 apresenta uma revisão bibliográfica sobre Aprendizado de Máquina aplicado à SRs baseados em conteúdo textual; por fim, o capítulo 4 apresenta em detalhes a proposta desse projeto de mestrado, um relato sobre os resultados já obtidos e o cronograma que guia a realização das próximas atividades.


\chapter{Conceitos Fundamentais}

Este capítulo introduz os conceitos fundamentais para o entendimento da proposta de trabalho de mestrado apresentada neste documento.
Conceitos nas áreas de Sistemas de Recomendação (Seção~\ref{sec:sr}), Biclusterização (Seção~\ref{sec:bicl}) e Mineração de Texto (Seção~\ref{sec:mintexto}) são apresentados.
...
Formalmente, \citeonline{Burke2002,Burke2007} define SRs como um conjunto de itens ${\cal I}$ que podem ser recomendados, um conjunto de usuários $U$ cujas preferências são conhecidas, um usuário $u$ pra o qual as recomendações são geradas, e algum item $i$ para o qual se quer predizer a preferência para $u$.
\citeonline{Adomavicius2005} extendem a definição de SR com uma função de utilidade $f$ que mede o quão útil é o item $i$ para o usuário $u$: $f: {\cal I} \times U \rightarrow R$, em que $R = \{ r_{u_1,I_1}, \dots, r_{u_1,I_m}, \dots, r_{u_1,I_M}, \dots, r_{u_n,I_1}, \dots, r_{u_n,I_M}, \dots, r_{u_N,I_1}, \dots, r_{u_N,I_m}, \dots,\\ r_{u_N, I_M} \}$ é um conjunto ordenado com valores faltantes, sendo $r_{u_n,i_m}$ um inteiro ou real que representa a interação do usuário $u_n$ no item $i_m$.
...
SRs baseados em conteúdo sofrem desse problema pois combinam o perfil de preferências de um usuário com os itens em ${\cal I}$, restrigindo o espaço de busca para realizar a sugestão de itens.
...
Os SRs baseados em filtragem colaborativa são os únicos que sofrem do problema de \textit{esparsidade}, que é o fato de usuários interagirem com apenas um pequeno subconjunto do conjunto de itens, tornando a matriz de interações ou preferências ($U \times {\cal I}$) esparsa.
...
Formalmente, há um conjunto de documentos $D = \{ d_1, d_2, \dots, d_n \}$, em que $d_i$ representa um documento e $n$ o número total de documentos, e um conjunto de termos ${\cal T} = \{ t_1, t_2, \dots, t_m \}$, em que $t_j$ representa um termo e $m$ o número de termos presentes em todos os documentos.
...
Para então, realizar a implementação de algoritmos de Biclusterização, que formarão biclusters para recomendar aos usuários, com base nos seus respectivos históricos ${\cal H}_u$.
...
A base de dados de cliques iG, doada para a realização deste trabalho, é composta por um conjunto usuários anônimos $U = \{ u_1, \dots, u_n, \dots, u_N \}$ que foram capturados através do controle de \textit{cookies} dos navegadores do portal, assim, cada usuário $u \in U$ interage com o conjunto de notícias ${\cal N}$ através de cliques, representados por $h_{u_n,I_m}^t$, um clique em uma notícia $I_m$ que foi dado por $u_n$ em um dado momento do tempo $t$.
Assim, se considerar cada clique $h_{u_n,I_m}^t$ como uma preferência do usuário $u_n$ por $n$, é possível construir a matriz de preferências $U \times I$ (Seção~\ref{sec:sr}), no contexto, ${\cal I} = {\cal N}$.
...
\begin{ganttchart}[
  ...
  \ganttbar[progress=0]{[1.3]}{1}{3} \\
  \ganttbar[progress=0]{[1.4]}{1}{9} \\

  \ganttgroup[progress=80]{[2]}{1}{7.5} \\
  \ganttbar[progress=95]{[2.1]}{1}{1.5} \\
  \ganttbar[progress=20]{[2.2]}{2}{4} \\

  \ganttgroup[progress=15]{[3]}{1}{27} \\
  \ganttbar[progress=45]{[3.1]}{1}{24} \\
  \ganttbar[progress=00]{[3.2]}{1}{21} \\
  \ganttbar[progress=20]{[3.3]}{2}{24} \\
  \ganttbar[progress=00]{[3.4]}{6}{27} \\

  \ganttgroup[progress=0]{[4]}{12}{18} \\
  \ganttbar[progress=00]{[4.1]}{12}{18} \\
  \ganttbar[progress=00]{[4.2]}{12}{18} \\
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item[{\texttt{[1.3]}}] Elaboração de um artigo, considerando os estudos já realizados, com o intuito de publicação no evento \textit{X Simpósio Brasileiro de Sistemas de Informação}\footnote{http://www.sbsi2014.uel.br/} (SBSI).
    \item[{\texttt{[1.4]}}] Elaboração de um artigo, com o intuito de publicação na revista \textit{User Modeling and User-Adapted Interaction}\footnote{\textit{http://www.umuai.org/}} (UMUAI).
  \end{enumerate}
  ...
  \end{enumerate}
  \item[{\texttt{[3]}}] Estudo e aplicação de técnicas de Biclusterização.
  \begin{enumerate}
    \item[{\texttt{[3.1]}}] Estudo das técnicas de Biclusterização, incluindo algoritmos e formas de avaliação.
    \item[{\texttt{[3.2]}}] Implementação de outras técnicas e algoritmos de Biclusterização
  ...
  \end{enumerate}
  \item[{\texttt{[4]}}] Estudos de possíveis extensões para o trabalho.
  \begin{enumerate}
    \item[{\texttt{[4.1]}}] Estudo de técnicas para recomendação utilizando modelos escondidos de Markov.
    \item[{\texttt{[4.2]}}] Estudo de técnicas de \textit{ensemble} de clusterização para uma representação alternativa dos itens.
  \end{enumerate}
...
\chapter{Exemplo de protocolo de condução para uma base de dados}
\label{app:protocolo}

\section*{Scopus}

\paragraph*{\textbf{\textit{String} de busca:}}
(TITLE-ABS-KEY("content based filter" OR "content based filters" OR "content based filtering" OR "content based recommender" OR "content based recommendation" OR "content based recommenders" OR "content-based filter" OR "content-based filters" OR "content-based filtering" OR "content-based recommender" OR "content-based recommendation" OR "content-based recommenders")) AND (TITLE-ABS-KEY(text OR textual OR news) OR TITLE(article OR articles OR paper OR papers OR book OR books) OR KEY(article OR articles OR paper OR papers OR book OR books)) AND (LIMIT-TO(PUBYEAR, 2014) OR LIMIT-TO(PUBYEAR, 2013) OR LIMIT-TO(PUBYEAR, 2012) OR LIMIT-TO(PUBYEAR, 2011) OR LIMIT-TO(PUBYEAR, 2010) OR LIMIT-TO(PUBYEAR, 2009) OR LIMIT-TO(PUBYEAR, 2008) OR LIMIT-TO(PUBYEAR, 2007) OR LIMIT-TO(PUBYEAR, 2006) OR LIMIT-TO(PUBYEAR, 2005) OR LIMIT-TO(PUBYEAR, 2004) OR LIMIT-TO(PUBYEAR, 2003)) AND (LIMIT-TO(LANGUAGE, "English"))

\paragraph*{\textbf{Filtro de Data de Publicação:}} 2003 - 2014

\paragraph*{\textbf{Data da Busca:}} August 30, 2014

\paragraph*{\textbf{Número de Resultados:}} 126

\paragraph*{\textbf{Filtro por Idioma:}} Inglês


\chapter{Exemplo de protocolo de extração para um trabalho incluído}
\label{app:protocolo-extracao}

\section*{~\cite{Bouras2014}}

\begin{tabular}{|>{\centering}m{3cm}|p{10cm}|}
\hline
  Índice & \#918 \\
\hline
  Título & Assisting cluster coherency via n-grams and clustering as a tool to deal with the new user problem \\
\hline
  Autores & Christos Bouras, Vassilis Tsogkas \\
\hline
  Veículo de publicacão: & International Journal of Machine Learning and Cybernetics \\
\hline
 Ano & 2014 \\
\hline
 Responde quais questões? & 1, 2ab, 3, 4 \\
\hline
 Contexto do Sistema de Recomendação & Notícias \\
\hline
\end{tabular}
...
\begin{tabular}{|>{\centering}m{3cm}|p{10cm}|}
\hline
 Resumo & Propõem uma abordagem para o problema de \textit{user cold-start}, faz o uso de classes/entidades extraídas do \textit{WordNet} e \textit{n-grams} dos termos (\textit{keywords}) extraídos das notícias com \textit{POS tag}. É usado um algoritmo de de clustering W-kmeans para clusterização de notícias e perfis de usuários. Foram feitos testes considerando o número de artigos que eram apresentados para o usuário classificar um número $R_{min}$ de artigos classificados (online). MAE para determinar o número de artigos que serão apresentados. Foram feitos testes comparando diversas estratégias vs a proposta de clustering. Além disso, são realizados \textit{testes offline} usando medidas de avaliação de clustering para determinar o melhor $n$ do \textit{n-gram}, e os melhores pesos $A,B$: $A*BagOfWords$ e $B*ngram$, W-kmeans vs kmeans. \\
\hline
\end{tabular}
...
\begin{tabular}{|>{\centering}m{3cm}|p{10cm}|}
\hline
 Conjunto de Dados & $10\;000$ notícias obtidos de portais como BBC, CNN, etc. durante um período de $5$ meses \\
\hline
\end{tabular}

\bigskip

\begin{tabular}{|>{\centering}m{3cm}|>{\centering}m{2cm}|p{7.6cm}|}
\hline
 \multicolumn{3}{c}{Analisador de Contexto} \\
\hline
 Representação textual & \multicolumn{2}{p{9.6cm}|}{ \textit{POS tag}, \textit{WordNet hypernyms}, TF-IDF } \\
\hline
 Pré-processamento & \multicolumn{2}{p{9.6cm}|}{ \textit{stemming}, \textit{stopwords}, filtro de tokens com baixas frequências } \\
\hline
 Redução de dimensionalidade & \multicolumn{2}{c|}{ - } \\
\hline
 \multirow{4}{*}{\parbox{3cm}{\centering Tarefa de Aprendizado de Máquina}}
& clusterização & Notícias são representadas através de clusters, formados pelo W-kmeans \\
& classicacão & - \\
& associação & - \\
& regressão & - \\
\hline
...
\begin{tabular}{|>{\centering}m{3cm}|>{\centering}m{2cm}|p{7.6cm}|}
\hline
\multicolumn{3}{c}{Aprendizado do perfil} \\
\hline
 Representação do perfil & \multicolumn{2}{c|}{ Tupla $(noticia, nota[0-5])$ } \\
\hline
 \multirow{4}{*}{\parbox{3cm}{\centering Tarefa de Aprendizado de Máquina}}
& clusterização & - \\
& classicacão & É realizado clustering dos perfis usando W-kmeans \\
& associação & - \\
& regressão & - \\
\hline
\end{tabular}
...
\begin{tabular}{|>{\centering}m{3cm}|p{10cm}|}
\hline
 \multicolumn{2}{c}{Componente de Filtragem} \\
\hline
  Estratégia & Lista de artigos é apresentada ao usuário -> Artigo é classificado pelo usuário -> busca no cluster desse artigo mais artigos; Se nenhum artigo é classificado -> busca usuários que deram nota para esses artigos apresntados -> apresenta a lista de artigos que esses usuários deram maiores notas; Se algum artigo é classificado -> busca nos clusters de usuários perfis semelhantes -> apresenta a lista de artigos que esses usuários deram maiores notas \\
\hline
\end{tabular}
...
\begin{tabular}{|>{\centering}m{3cm}|p{10cm}|}
\hline
 \multicolumn{2}{c}{Teste} \\
\hline
  Métricas & \textit{MAE}, \textit{Cluster Index} e Ad-Hoc (parecido com PR) \\
\hline  
  Estratégia & Online: \textit{MAE} para achar o número de artigos de um mesmo cluster para serem apresentados; o número de artigos apresentados para alcançar $R_{min}$ classificações, comparando diferentes estratégias - entropy, random, popularity, balanced, personalized, clustering; \textit{MAE} para cada uma das estratégias. Offline: determinar o \textit{n} do \textit{n-gram} e $A,B$ (parâmetros de pesagem) usando a medida de clustering CI. \\
\hline
\end{tabular}
...
\begin{tabular}{|>{\centering}m{3cm}|p{10cm}|}
\hline
 Resultados & A estratégia adotada mostrou ser melhor que as demais, que são comumente usadas. Com os experimentos, $5$ é o número ideal de artigos para serem mostrado aos usuários. Em média $32,3$ artigos devem ser sugeridos para obter $20$ artigos classificados. $n = 2$ e $3$ é mais performático e $30\%$ de \textit{n-grams} para a representação (peso $B$). \\
\hline
 Notas & O artigo é bem completo. Apresenta um SR híbrido. Boas formas de avaliar a performance. Porém não avalia em termos de MAE se o \textit{n-gram} influencia na recomendação. \\
 \hline
\end{tabular}
