...
\citeonline{Ricci2011} argumenta que muitos usuários não tem conhecimento suficiente para saber o que querem ou não, consumir da web.
...
% Sendo assim, este trabalho tem o intuito de aplicar técnicas e algoritmos em SRs que se baseiam no conteúdo das notícias para propor recomendações que sejam diversificadas e úteis para o usuário.

\section{Apresentação do Problema de Recomendação}

% Apresentar de maneira mais formal (acima foi só um cenário), o problema de recomendação. Tentar caracterizar (aí acho que de maneira não formal necessariamente) a propriedade de \textit{serendipidade}.
...
% Apresentar a nossa hipótese de que se juntar biclusterização com recomendação vai resolver \textit{serendipidade}.
...
\caption{Arquitetura de um SRbC (Adaptado de \citeonline{Lops2011}).}
\label{fig:approach}
\end{figure}
