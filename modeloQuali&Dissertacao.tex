...
% Lucas, eu achei que seu texto estava trocando os interesses em relação ao exemplo, por isso troquei.

Uma melhoria para o sistema hipotético (Figura~\ref{fig:story}) seria dotá-lo da capacidade de perceber, por exemplo, que notícias sobre esporte podem trazer conteúdo interessante sobre música, recomendando uma notícia ao usuário interessado em música que, originalmente, só seria recomendada ao usuário interessado em esporte. Por exemplo, é sabido que eventos de beisebol -- o superbowl -- possuem uma abertura cultural na qual grandes artistas da música fazem apresentações. Assim, notícias inicialmente classificados como sendo de esporte poderiam interessar a quem gosta de música, desde que apresentem informações atraentes sobre música. % não acho que cabe exatamente falar "vice-versa" aqui. Teria que inverter o exemplo dando uma notícia de música onde esporte é comentado de alguma forma.
...
% acho que tem que falar de biclusterização nessa primeira seção, senão a biclusterização vai ficar totalmente jogada mais para frente.
...
% vale colocar uma referência para bilclusterização aqui. Acho que o ideal é o artigo que discute os algoritmos de biclustering e algo mais antigo, como um artigo onde o termo foi apresentado. 


\section{Apresentação do Problema de Recomendação}

% Lucas, embora eu ache que não está errado do jeito que você fez, eu vou sugerir um outro jeito. Você escolhe o que lhe deixa mais confortável.
%Seja um conjunto de usuários $U = \{ u_1, \dots, u_n \}$, um conjunto de notícias ${\cal N} = \{ N_1, N_2, \dots, N_m \}$  e o histórico $h_{u_i,N_j} \in {\cal H}$ indicando se o usuário $u_i$ leu a notícia $N_j$.O problema de recomendação de notícias, com base no contexto, é a definição de uma função de similaridade entre notícias $s: N \rightarrow N \times N$ para aprender uma função $l: {\cal H}_u, s \rightarrow L_u$, onde ${\cal H}_u$ é o subconjunto que representa o histórico de notícias que $u$ leu, e $L_u$ a lista de recomendações de notícias direcionadas à $u$.

% vc não usou os termos aqui, então não precisa definir, ficou jogado.
% que podem ser representadas, por exemplo, como um conjunto de termos $N = \{ t_1, t_2, \dots \}$,

% é conteúdo e não contexto né?
...
% acho que usar N e M aqui e tambem n item 1.2 pode ser um problema. Mas não vou trabalhar tanto na notação agora. Se houver tempo, você muda o N e o M daqui.
...
% Lucas, não sei se ficou bom a questão da serendipidade, mas foi o melhor que consegui fazer no tempo que tenho. 

% Outra observação. Não acho que você deva tratar de definições usando NOTICIAS e sim ITENS, pois seu trabalho não está necessariamente preocupado com notícias e sim com TEXTO. As noticias são só um pretexto de experimentação e de motivação.

% Isso deveria ficar mais bem delineado no documento. Por isso não se deixa a escrita para fazer em cima da hora, ela precisa de maturação, o que não haverá na sua quali. Espero que para o texto da dissertação isso possa ocorrer.


\section{Apresentação do contexto de Biclusterização}

% Se você aceitar a notação que eu fiz na parte de cima, tem que aceitar aqui para ficar padrão. Se vc não aceitar, volte aqui no que era antes.
...
% não tem o "ou não" ele sempre considera, talvez no resultado não aconteça nada nos atributos, e o resultado seja como a clusterização, mas ele considerou. Se o resultado ficou igual de clusterização é porque tinha que ser assim.

% eu não gostei dessa definição de biclusters. Não entendi o "condicionalmente" e não encontrei a questão da correlação parcial ou similaridade parcial. Além disso, existem definições já maduras na literatura. Você deve usar uma delas. Sugiro que você coloque PELO MENOS uma definição com base na tese do Fabrício, afinal ele estará na sua banca.

% ALIAS, ACABEI DE OLHAR AS SUAS REFERÊNCIAS, PELO MENOS AS QUE ESTÃO NO PDF GERADO POR ESTE TEX E O FABRICIO NÃO ENTROU. NAO DÁ PARA CHAMAR UMA PESSOA QUE TEM DOUTORANDO EM BICLUSTERING PARA BANCA SEM QUE ELE TENHA SIDO CITADO NO TRABALHO. NAO ENTREGUE O DOCUMENTO DE QUALIFICACAO SEM FAZER CITACOES À TESE DO FABRICIO (E ARTIGOS DELES SE FOR POSSIVEL).

%Achei sua definição de bicluster tão fraca que não há como entender dela do que se trata. Ou uma definição mais forte e mais detalhada é colocada aqui, ou você vai ter que colocar um exemplo gráfico ou numérico (como tem na tese do Fabrício).
...
% acho que esse parágrafo ficou perdido aqui, não vejo necessidade dele. 
...
% Deixei o original aqui para depois, com calma, vc ver o parágrafo que vc tinha feito. Como ele está sem sentido. Imagino que sua pressa está tão grande que não houve tempo de você reler seus textos. Veja o problema! Eu não vou ter como ler o seu texto completo então acho que voce preciria tentar revisar com cuidado.
...
% vc acha que serendipidade precisa ficar em itálico? eu acho que não precisa, mas se quiser pode deixar, apenas tenha um motivo para isso.
...
% LUCAS, LEIA E RELEIA A HIPOTESE MUITAS VEZES. VEJA SE PODE MELHORAR. ESSA ITEM, SEGUIDO PELOS OBJETIVOS, VAI SER O MAIS VISADO PELOS MEMBROS DA BANCA.

\section{Objetivos}

% Colocar o objetivo de resolver o problema acima, otimizando a \textit{serendipidade}, através do uso de biclusterização nos documentos (notícias).
...
ˆ% Veja com calma esses objetivos também. Eu mudei um pouco tentando deixá-los um pouco mais "evoluídos" e menos cumprindo tabela.
...
% Aqui explicar cada um dos passos. Explicar que o levantamento do referencial teórico através de análise exploratória. Explicar como é a revisão sistemática (um parágrafo). Explicar a motivação de tê-la realizado.
...
%verifique como mencionar stemming em portugues e arrume aqui. Eu improvisei.
%revise o parágrafo e veja o que mais deve ser dito.
...
% Explicar que as técnica de biclusterização e essemble serão aplicadas aos textos processados e que uma avaliação será feita com base nas informações de cliques.
...
\citeonline{Feldman2006}, apresentam uma arquitetura geral para aplicações de Mineração de Textos composta por quatro etapas: tarefas de pré-processamento, que preparam os dados para a central de operações de mineração; central de operações de mineração, incluem algoritmos para a descoberta de padrões, tendências e conhecimentos através de técnicas e algoritmos; componentes de apresentação, incluem interfaces para o usuário apresentando visualizações dos conhecimentos gerados na etapa anterior; e técnicas de refinamento, também descrito como uma fase de pós-processamento, que inclui métodos para filtrar informações redundantes.
    ...
    \citeonline{Salton1975} argumentam que a representação textual de documentos em vetor de palavras é suficiente para separar documentos.
...
\caption{Arquitetura de um CBRS} %(Adaptado de \cite{Lops2011}).} %LUCAS, RETIREI ISSO
\label{fig:approach}
\end{figure}
